...
1. **A traffic-based onboarding gate is in place.** Synchronizer access must be gated so that participants incur a traffic cost to onboard, and allowing the blocking of nodes at a protocol level.
...
With a traffic-based onboarding gate in place, the IP whitelist is no longer needed to have control over the participants that can connect to the synchronizer. 
